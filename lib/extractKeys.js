	...
	try {
		...
		if (typeof element === 'string' || typeof element === 'number') {
			...
			if (!alreadyCreatedOBjects[path]) {
				//const roleType = getRoleType(element,write);
				await adapter
					.extendObject(path, {
						type: 'state',
						common: {
...
function getRole(element, write) {
	if (typeof element === 'boolean') {
		if (write) return 'switch';
		return 'indicator';
	}

	if (typeof element === 'number' || isNumber(element)) {
		if (write) return 'level';
		return 'value';
	}

	if (isDate(element)) return 'date';
