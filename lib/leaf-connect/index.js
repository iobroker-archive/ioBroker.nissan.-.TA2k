...
module.exports = async ({
	...
	locale = 'en-US',
	debug = false,
	pollingInterval = RESULT_POLLING_INTERVAL,
}) => {
	if (!username) {
	...
	// @ts-expect-error axios create method
	...
	return {
		...
		status: async () => {
			const { data } = await apiInstance({
				url: 'BatteryStatusCheckRequest.php',
				data: sessionState.requestOptions,
			});
		...
		cachedStatus: async () => {
			const { data } = await apiInstance({
				url: 'BatteryStatusRecordsRequest.php',
				data: sessionState.requestOptions,
			});
		...
		climateControlStatus: async () => {
			const { data } = await apiInstance({
				url: 'RemoteACRecordsRequest.php',
				data: sessionState.requestOptions,
			});
		...
		chargingStart: async () => {
			const { data } = await apiInstance({
				url: 'BatteryRemoteChargingRequest.php',
				data: sessionState.requestOptions,
			});
